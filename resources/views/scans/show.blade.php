                ...
                <div class="lg:w-2/3 flex flex-col gap-4">
                    ...
                    <div
                        ...
                        <div class="absolute bottom-4 left-4 text-gray-400 text-xs">
                            {{ __('Scanned on') }} {{ $scan->created_at->format('d/m/Y') }} {{ __('at') }} {{ $scan->created_at->format('H:i') }}
                        </div>
                    </div>
                        ...
                        <div x-show="open" class="mt-4 pt-4 border-t border-gray-100" style="display: none;"
                            ...
                            <form action="{{ route('scans.updateImage', $scan->id) }}" method="POST"
                                ...
                                <div class="flex items-center gap-4">
                                    <div class="flex-grow">
                                        <label class="block text-xs font-bold text-gray-500 uppercase mb-1">{{ __('New image') }}</label>
                ...
                <div class="lg:w-1/3 flex flex-col gap-6 overflow-y-auto">
                    ...
                    <div
                        ...
                        <div class="flex justify-between items-start mb-2">
                            <h3 class="text-gray-500 text-xs font-bold uppercase tracking-wider">{{ __('Artificial Intelligence Analysis') }}</h3>
                        ...
                        @if ($scan->ai_result)
                            ...
                            <p class="text-sm text-gray-600">{{ __('Automatic detection based on Deep Learning model.') }}
                            </p>
                        @else
                            ...
                            <p class="text-xs text-gray-400 mt-2">{{ __('The Python script has not processed this image yet.') }}
                            </p>
                        @endif
                    </div>

                    <div class="bg-white p-6 rounded-lg shadow-md flex-grow">
                        <h3 class="text-gray-500 text-xs font-bold uppercase tracking-wider mb-4">{{ __('Clinical Validation') }}
                        </h3>
                        ...
                        <form action="{{ route('scans.update', $scan->id) }}" method="POST">
                            ...
                            <div class="mb-4">
                                <label class="block font-bold text-gray-700 text-sm mb-2">{{ __('Final Diagnosis') }}</label>
                                <select name="final_diagnosis"
                                    class="w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                    <option value="">{{ __('Select...') }}</option>
                                    <option value="Pas de Rétinopathie"
                                        {{ $scan->final_diagnosis == 'Pas de Rétinopathie' ? 'selected' : '' }}>{{ __('No Retinopathy (Healthy)') }}</option>
                                    <option value="Légère" {{ $scan->final_diagnosis == 'Légère' ? 'selected' : '' }}>
                                        {{ __('Mild Retinopathy') }}</option>
                                    <option value="Modérée"
                                        {{ $scan->final_diagnosis == 'Modérée' ? 'selected' : '' }}>{{ __('Moderate Retinopathy') }}</option>
                                    <option value="Sévère" {{ $scan->final_diagnosis == 'Sévère' ? 'selected' : '' }}>
                                        {{ __('Severe Retinopathy') }}</option>
                                    <option value="Proliférante"
                                        {{ $scan->final_diagnosis == 'Proliférante' ? 'selected' : '' }}>{{ __('Proliferative Retinopathy') }}</option>
                                </select>
                            </div>

                            <div class="mb-4">
                                <label class="block font-bold text-gray-700 text-sm mb-2">{{ __('Prescription / Treatment') }}</label>
                            ...
                            <div class="mb-6">
                                <label class="block font-bold text-gray-700 text-sm mb-2">{{ __('Private Notes') }}</label>
                    ...
                    @if ($scan->status == 'valide')
                        <div class="bg-gray-50 p-4 rounded-lg border border-gray-200 text-center">
                            <p class="text-sm text-gray-600 mb-3">{{ __('Diagnosis is validated.') }}</p>
                            <button onclick="alert('{{ __('We will install the PDF generator in the next step!') }}')"
